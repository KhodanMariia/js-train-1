  ...
  if (num <= 1) {
    ...
    for (let i = 2; i <= num; i++) {
      ...
      if (num % i === 1) {
        ...
        return false; // Якщо так, то num не є простим, тому повертаємо false
      }
...
function getVariableType(variable) {
  ...
  return variable == `number` // Якщо тип змінної - number,
    ? (variable = `це число`) //то повідомлення = "Це число"
    : variable == `string` // Якщо тип змінної - string,
    ? (variable = `це рядок`) //то повідомлення = "Це рядок"
    : variable == `boolean` // Якщо тип змінної - boolean,
    ? (variable = `це булеве значення`) //то повідомлення = "Це булеве значення"
    : (variable = `це інший тип`); // В іншому випадку повідомлення = "Це інший тип"
...
function factorial(n) {
  // Якщо n дорівнює 0 або 1,
  if (n == 1) {
    return 1; //повертаємо 1, оскільки факторіал 0 і 1 завжди дорівнює 1.
  } else {
    // Якщо n більше за 1,
    return n * factorial(n - 1); // ми множимо n на результат виклику тієї самої функції з n-1.
  } //Факторіал числа n – це добуток всіх цілих чисел від 1 до n.
  //Наприклад, факторіал числа 5 дорівнює 1 * 2 * 3 * 4 * 5 = 120.
}
...
// Створюємо змінну getAdder якій присвоємо результат виконання
// функції makeAdder з аргументом 5
...
function memoizedArea() {
  ...
  return function (side) {
    if (side == a) {
      ...
      return r; // Повертаємо попереднє значення
    }

    if (side !== a) {
      // Якщо сторона відрізняється від попередньої,
      r = side * side; //обчислюємо площу
      a = side; //та зберігаємо результат та аргумент для майбутнього використання
    ...
    // Розраховуємо площу
    let prevSide = side; // prevSide присвоюємо сторону квадрата
    let prevResult = prevSide * prevSide; // prevResult просвоюємо площу квадрата

    return prevResult; // Повертаємо площу
  };
}
